<script lang="ts">
	...
	const product = {
		...
		price: 29.99,
		description:
			'Our signature t-shirt combines style and comfort with premium organic cotton. Perfect for everyday wear with a relaxed fit and reinforced stitching for durability.',
		features: ['100% Organic Cotton', 'Relaxed Fit', 'Machine Washable', 'Available in 5 colors'],
		images: [
	...
	const reviews = [
		{
			...
			author: 'Alex M.',
			rating: 5,
			text: 'Absolutely love this shirt! The fabric is so soft and comfortable, and it fits perfectly. Will definitely buy more colors.'
		},
		{
			id: 2,
			author: 'Jamie L.',
			rating: 4,
			text: 'Great quality t-shirt that has held up well after multiple washes. The fit is slightly larger than expected, but still looks good.'
		},
		{
			id: 3,
			author: 'Taylor S.',
			rating: 5,
			text: 'This is my go-to t-shirt now. The material feels premium and breathable. Worth every penny!'
		}
	];
	...
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
		<div class="grid grid-cols-1 md:grid-cols-2 gap-8">
			...
			<div class="flex flex-col">
				...
				<div class="flex space-x-4 overflow-x-auto pb-2">
					{#each product.images as image}
						<button
							class="border rounded-md overflow-hidden flex-shrink-0 {selectedImage.id === image.id
			...
			<div class="flex flex-col">
				...
				<div class="mt-6">
					<button
						class="w-full bg-rose-600 border border-transparent rounded-md py-3 px-8 flex items-center justify-center text-base font-medium text-white hover:bg-rose-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-rose-500"
					>
						Add to Cart
					</button>
				</div>
