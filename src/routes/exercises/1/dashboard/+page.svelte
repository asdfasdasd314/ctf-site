<script lang="ts">
	...
	onMount(async () => {
		...
		const userCookie = cookies.find((cookie) =>
			cookie.trim().startsWith('vuln_auth_public_id=')
		);
		...
		const public_id = userCookie.split('=')[1];
...
<div class="min-h-screen bg-gray-100">
	...
	<header class="bg-emerald-800 shadow">
		<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
			<h1 class="text-xl font-bold text-white">Super Secure Dashboard</h1>
			<button
				onclick={signOut}
				...
				<div class="border-t border-gray-200">
					<dl>
						...
						<div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
							...
							<dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{last_name}</dd>
						</div>
					</dl>
				</div>
			</div>
