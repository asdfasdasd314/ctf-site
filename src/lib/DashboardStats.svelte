<script lang="ts">
	...
	import { type Exercise, getExercises, getExerciseCompletions } from './exercises';
	import { supabase } from './supabaseClient';

	// Solved stats for each category
	...
	// Solved stats for each difficulty
	...
	let categoryStats = $state<Record<number, CategoryStats>>({});
	let difficultyStats = $state<Record<number, DifficultyStats>>({});
	...
	onMount(async () => {
		try {
			...
			const { data: categories, error: categoriesError } = await supabase.from('categories').select('category,display_color,category_id');
			if (categoriesError) {
				console.error('Error loading categories:', categoriesError);
				return;
			}

			const { data: difficulties, error: difficultiesError } = await supabase.from('difficulties').select('difficulty,display_color,difficulty_id');
			if (difficultiesError) {
				console.error('Error loading difficulties:', difficultiesError);
				return;
			}

			const { data: completions, error: completionsError } = await supabase.from('completions').select('exercise_id,created_at');
			if (completionsError) {
			...
			try {
				rawExercises = await getExercises();
			}
			catch (error) {
				console.error('Error fetching exercises:', error);
			}

			const prevMidnight = new Date((new Date()).toISOString().slice(0,10) + 'T00:00:00').getTime() / 1000 - 24 * 60 * 60;
			let lowestTimestamp = prevMidnight;
			let highestTimestamp = prevMidnight + 24 * 60 * 60;

			completions.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
			const firstCompletionTimestamp = new Date(completions[0].created_at).getTime() / 1000;
			if (firstCompletionTimestamp > lowestTimestamp && firstCompletionTimestamp < highestTimestamp) {
				streak++;
			}

			highestTimestamp = lowestTimestamp;
			lowestTimestamp -= 24 * 60 * 60;

			for (let i = 1; i < completions.length; i++) {
				const completion = completions[i];
				const completionTimestamp = new Date(completion.created_at).getTime() / 1000;
				if (completionTimestamp > highestTimestamp) {
					continue;
				}
				if (completionTimestamp < lowestTimestamp) {
					break;
				}
				streak++;
				highestTimestamp = lowestTimestamp;
			...
			// Get the exercise categories and difficulties
			for (const exercise of rawExercises) {
				const category = categories.find(c => c.category_id === exercise.categoryId);
				const difficulty = difficulties.find(d => d.difficulty_id === exercise.difficultyId);
				if (category && difficulty) {
					if (!categoryStats[category.category_id]) {
					...
					if (!difficultyStats[difficulty.difficulty_id]) {
						difficultyStats[difficulty.difficulty_id] = {
							name: difficulty.difficulty,
							color: difficulty.display_color,
							total: 0,
							solved: 0,
						};
					}
					categoryStats[category.category_id].total++;
					if (completions.some(c => c.exercise_id === exercise.exerciseId)) {
						categoryStats[category.category_id].solved++;
					}
					difficultyStats[difficulty.difficulty_id].total++;
					if (completions.some(c => c.exercise_id === exercise.exerciseId)) {
						difficultyStats[difficulty.difficulty_id].solved++;
					}
				}
				else {
					console.error('Error getting category or difficulty for exercise:', exercise);
					continue;
				}
				totalExercises++;
				if (completions.some(c => c.exercise_id === exercise.exerciseId)) {
...
<div class="dashboard-stats bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
				...
				<div class="bg-white dark:bg-gray-800 rounded-lg p-4 shadow-sm">
					<div class="flex items-center">
						...
						<div>
							...
							<p class="text-xl font-bold text-gray-800 dark:text-white">{totalPoints}</p>
						</div>
					</div>
				</div>
				...
				<div class="bg-white dark:bg-gray-800 rounded-lg p-4 shadow-sm">
					<div class="flex items-center">
						...
						<div>
							...
							<p class="text-xl font-bold text-gray-800 dark:text-white">{streak} days</p>
						</div>
					</div>
				</div>
	...
	<div class="bg-gray-50 dark:bg-gray-700 rounded-xl p-6 mb-8">
		...
		<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
			{#each Object.values(categoryStats) as category}
				<div class="bg-white dark:bg-gray-800 rounded-lg p-4 shadow-sm">
	...
	<div class="bg-gray-50 dark:bg-gray-700 rounded-xl p-6 mb-8">
		...
		<div class="space-y-4">
			{#each Object.values(difficultyStats) as difficulty}
				<div>
					<div class="flex justify-between items-center mb-2">
						<div class="flex items-center">
							<div
					...
					<div class="w-full bg-gray-200 dark:bg-gray-600 rounded-full h-2.5">
						<div
							class="h-2.5 rounded-full transition-all duration-1000 ease-out"
							style="background-color: {difficulty.color}; width: {difficulty.total > 0 ? (difficulty.solved / difficulty.total) * 100 : 0}%;"
						></div>
					</div>
				</div>
			{/each}
	...
	<div class="bg-gray-50 dark:bg-gray-700 rounded-xl p-6">
		...
		<div class="flex flex-col md:flex-row items-center gap-6">
			...
			<div class="grid grid-cols-2 gap-x-8 gap-y-2">
				{#each Object.values(categoryStats).filter((cat) => cat.solved > 0) as category}
					<div class="flex items-center">
	...
	<div class="bg-gray-50 dark:bg-gray-700 rounded-xl p-6">
		<div class="grid grid-cols-2 md:grid-cols-4 gap-4">
			<div class="text-center">
				<div class="text-2xl font-bold text-gray-800 dark:text-white">{totalPoints}</div>
				<div class="text-sm text-gray-600 dark:text-gray-400">Total Points</div>
			</div>
			<div class="text-center">
				<div class="text-2xl font-bold text-gray-800 dark:text-white">{totalSolved}</div>
			...
			<div class="text-center">
				<div class="text-2xl font-bold text-gray-800 dark:text-white">{formatRelativeTime(createdAt)}</div>
